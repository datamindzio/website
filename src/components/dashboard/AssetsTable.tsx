...
export default function AssetsTable() {
  return (
    <div className="card assets-table" style={{ overflow: 'hidden', borderColor: 'var(--border-dash)' }}>
      ...
      <div className="assets-table__scroll">
        <div className="mono assets-table__row" style={{ display: 'grid', gridTemplateColumns: COLS, padding: '10px 24px', fontSize: 10, letterSpacing: '.1em', color: 'var(--text-dimmer)', borderBottom: '1px solid var(--divider-soft)' }}>
          <span>ASSET</span><span>TYPE</span><span>CAPACITY</span><span>OUTPUT (7D)</span><span>STATUS</span>
        </div>
        {assets.map((a) => (
          <div key={a.name} className="assets-table__row" style={{ display: 'grid', gridTemplateColumns: COLS, padding: '14px 24px', fontSize: 13.5, borderBottom: '1px solid var(--divider-softer)', alignItems: 'center' }}>
